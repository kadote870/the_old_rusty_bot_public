...
const log_creator = () => {
    if (archiver) {
        const logFileStream: fs.WriteStream = fs.createWriteStream('log.txt', {flags: 'a'});
    ...
    constructor(message: any) {
        if (message.author.username === STANDARD_USERS.player1) {
            if (kangaroo) {
                ...
                if (randomizer === 3) {
                    message.channel.send(get_random_value_from_array(variable.kangarooRage));
                } else {
                    message.channel.send(this.kangarooChain(randomNumber(1, 10)));
                }
            } else {
                message.channel.send(`Chciałbym wysłać pięknego kangura, ale **${get_random_value_from_array(variable.insults)}** nie życzy sobie tego ;(`);
            }
        }
    }

    kangarooChain(count: number):
        string {
