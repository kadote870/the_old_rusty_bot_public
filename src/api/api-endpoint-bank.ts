...
router.get('/api/bank', (req, res) => {
    db.all('SELECT id, userId, characterName, bankBalance FROM players', (err, row) => {
        if (err) {
...
router.get('/api/bank/:userId', (req, res) => {
    ...
    db.get('SELECT id, userId, characterName, bankBalance FROM players WHERE userId = ?', [userId], (err, row) => {
        if (err) {
            console.error('500 Internal Server Error:', err.message);
            res.status(500).json({error: '500 Internal Server Error.'});
        } else {
...
router.patch('/api/bank/:userId', (req, res) => {
    ...
    db.run('UPDATE players SET bankBalance=? WHERE userId = ?',
