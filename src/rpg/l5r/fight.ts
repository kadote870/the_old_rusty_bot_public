...
import {ADMIN_DISCORD_ID} from "../../administration/administartion";
...
export const fight_initiative = () => {
    ...
    const result: string = preResult.join('\n');
    return result;
}
        ...
        (async (): Promise<void> => {
            ...
            if (userMessage.startsWith('dmg plus')) {
                ...
                if (args.length === 3 && !isNaN(amount) && amount > 0 && !pattern.test(args[2])) {
                    ...
                    await message.channel.send(`**${user}**:game_die:${variable.codeBlock}Dodano ${amount} obrażeń.\n${woundResult}${variable.codeBlock}`);
                }
            }
            ...
            if (userMessage.startsWith('dmg minus')) {
                ...
                if (args.length === 3 && !isNaN(amount) && amount > 0 && !pattern.test(args[2])) {
                    if (actualDamageLvl >= amount) {
                        ...
                        await message.channel.send(`**${user}**:game_die:${variable.codeBlock}Odjęto ${amount} obrażeń.\n${woundResult}${variable.codeBlock}`);
                    } else {
                        const woundResult = await wound_level_calculator(userId, 0);
                        await message.channel.send(`**${user}**:game_die:${variable.codeBlock}Nie można przeprowadzić operacji:[${message.content}].\n${woundResult}\nMaksymalna możliwa wartość to: ${actualDamageLvl}${variable.codeBlock}`);
                    }
                }
            }
