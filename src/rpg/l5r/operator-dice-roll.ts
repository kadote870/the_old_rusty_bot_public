...
import { CharacterSchoolEnum } from './data/character-enums';
      ...
      (async (): Promise<void> => {
                  ...
                  if (howManyDice >= howManyKeep) {
                     if (howManyDice < 1 || howManyKeep < 1) {
                        ...
                        if (playerData) {
                           ...
                           let bonus = 0;
                           const result: string = await legendRoll(
                              diceSum,
                              howManyKeep,
                              playerData.woundsMathLevel,
                              bonus
                           );
         ...
         if (userMessage === prefixLower + 'atak') {
            ...
            if (playerData) {
               ...
               let bonus = 0;
               if (playerData.school === CharacterSchoolEnum.BushiHida) {
                  // Bushi Hida - Ranga 1: Droga Kraba
                  bonus += playerData.ziemia;
               }
               const attack = await legendRoll(diceSum, playerData.zrecznosc, playerData.woundsMathLevel, bonus);

               message.channel.send(
                  `**${user}**:crossed_swords:${variable.codeBlock}[${playerData.characterName}]\nKenjutsu: ${playerData.skillKenjutsu}, **Zręczność: ${playerData.zrecznosc}**\nATAK: ${attack}${variable.codeBlock}`
               );
            } else {
               message.channel.send(
                  `${variable.codeBlock}DB error during select player id: ${userId} - probably player was not found${variable.codeBlock}`
               );
            }
         }
         ...
         if (userMessage === prefixLower + 'dmg') {
            ...
            if (playerData) {
               ...
               const damage = await legendRoll(diceSum, playerData.dmgKeep, playerData.woundsMathLevel, bonus);

               message.channel.send(
                  `**${user}**:boom:${variable.codeBlock}[${playerData.characterName}] Broń: ${playerData.weaponName}\nSila: ${playerData.sila}, dmgRoll: ${playerData.dmgRoll}, **dmgKeep: ${playerData.dmgKeep}**\nOBRAŻENIA: ${damage}${variable.codeBlock}`
               );
            } else {
               message.channel.send(
                  `${variable.codeBlock}DB error during select player id: ${userId} - probably player was not found${variable.codeBlock}`
