        ...
        (async (): Promise<void> => {
            if (vampireStatus) {
                switch (message.content.toLowerCase()) {
                    case ('npc'):
                        new BotResponse(user, message, `${variable.codeBlock}${new VampireNpc().printer()}${variable.codeBlock}`);
                        break
                    case ('eccehomo'):
                    case ('ecce homo'):
                        new BotResponse(user, message, `${variable.codeBlock}${new VampireHumanNpc().printer()}${variable.codeBlock}`);
                        break
                }
            } else {
