                ...
                if (args.length === 2 && !isNaN(diceCount) && !pattern.test(args[1])) {
                    if (!vampireStatus) {
                        ...
                        if (diceCount < 1) {
                            ...
                            response = await vampire_the_masquerade_roll_dice_and_create_result_message(user, message, diceCount);
                            message.channel.send(response);
                        }
                    }
                }
...
async function vampire_roll_row(diceCount: number, minSingleRoll: number, maxSingleRoll: number): Promise<number[]> {
    // roll (howManyDice: number)d10
    const sortDesc = (a: number, b: number) => b - a;
    const diceRoll = (howManyDice: number) => {
        let rollResults: number[] = [];
...
async function vampire_the_masquerade_roll_dice_and_create_result_message(user: any, message: any, diceCount: number): Promise<string> {
    const common: number[] = await vampire_roll(diceCount);
    const helper: number[] = await vampire_roll_row(diceCount, rollHelper.helperMinSingleRoll, rollHelper.helperMaxSingleRoll);
    const distractor: number[] = await vampire_roll_row(diceCount, rollHelper.distractedMinSingleRoll, rollHelper.distractedMaxSingleRoll);
