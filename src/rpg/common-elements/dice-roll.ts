...
export const sumHighestResults = (min: number, max: number, numDraws: number, keepResults: number): any => {
    const results: number[] = generateRandomResults(min, max, numDraws);
        const topResults = results.slice(0, keepResults);
        const sum: number = topResults.reduce((acc, val) => acc + val, 0);
